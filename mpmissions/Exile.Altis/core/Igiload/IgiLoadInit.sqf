...
_null = [Player] execVM "core\IgiLoad\IgiLoad.sqf";
...

{
	if ((typeOf _x) in (IL_Supported_Vehicles_All)) then
	{
		...
		_null = [_x] execVM "core\IgiLoad\IgiLoad.sqf";
		waitUntil {scriptDone _null};
	};
} forEach (vehicles);

call compileFinal preprocessFileLineNumbers "core\IgiLoad\IgiLoadTaru.sqf";
