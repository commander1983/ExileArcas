...
if (VQI_HINTS_SNIPER == 1) then { hint "Questrel: SniperBuddy I"; };
	...
	while {"VQI_SniperBuddy_1" in (assignedItems player)} do {
		...
		_elevation	= round (((getPosASL player) select 2)*3.2808);	
		_humidity	= fog;
		_hydrometeor	= rain;
		...
		(_display displayCtrl 2007) ctrlSetText format["%1'",_elevation];
		(_display displayCtrl 2008) ctrlSetText (format["%1",(round(_humidity*100))]+"%");
		(_display displayCtrl 2009) ctrlSetText format[".%1in",(round(_hydrometeor*100))];
