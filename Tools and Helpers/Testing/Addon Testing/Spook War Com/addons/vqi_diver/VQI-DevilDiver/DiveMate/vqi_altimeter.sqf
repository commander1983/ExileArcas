...
_pAlt1 	= (getposATL player) select 2;
...
//{
	...
	scopeName "halo_loop";
	//if (true) then
	if (("Helmet_VQI_HALO" == Headgear player) && ("Vest_VQI_Stingray" == Vest player) && (("Uniform_VQI_Slipstream" == Uniform player) || ("U_B_Wetsuit" == Uniform player))) then
	{	
			
			
disableSerialization; 
11 cutRsc ["VQI_ALTIMETER_1", "PLAIN"];

			//while {true} do
			while {("Helmet_VQI_HALO" == Headgear player) && ("Vest_VQI_Stingray" == Vest player)} do //&& (("Uniform_VQI_Slipstream" == Uniform player) || ("U_B_Wetsuit" == Uniform player))} do
			{
...
//showWatch false;
showCompass false;
//setCaptive true; //Too much? Maintain FULL Stealth while below canopy? 
...
_pSaO2f = 0;
_windDir = windDir;
knots = 3.6 * sqrt ((wind select 0)*(wind select 0) + (wind select 1)*(wind select 1));
//KNOTS are approx - WIP?
...
(_display displayCtrl 1007) ctrlSetText format["%1~%2",(round(_windDir)),(round(knots/1.852))];
				switch (true) do
					{ 
						...
						case (_pAlt > 28000): { 	//Alt Temp based on Climate: XTREME Level				
				...
				switch (true) do
					...
					//Special Effects on Helmut/Goggles - Temperature, Atmosphere, etc. - WIP????????????
					//switch (true) do
					//{
					//	case ((_pAlt > 35500) && (_pAlt <= 36430)): {					
						//2 cutRsc ["GOGGLES_FROST_EFX", "PLAIN"];					
					//	};
			...
			//sleep 0.01;			
			};//Altimeter-Loop End - SYSTEM TOO SLOW in MP?
