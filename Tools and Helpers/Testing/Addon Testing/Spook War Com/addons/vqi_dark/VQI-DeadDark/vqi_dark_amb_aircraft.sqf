...
tpw_air_delay = 666;		//_this select 0; // delay until flybys start
tpw_air_time = 1200; 		//_this select 1; // maximum time between flybys
tpw_air_max = 3; 			//_this select 2; // maximum number of aircraft at a given time
tpw_air_heights = [50,200,500]; 	//_this select 3; // flying heights to randomly select
tpw_air_civexclude = 0; 	//_this select 4; // 0 = all aircraft, 1 = civilian aircraft excluded, 2 = military aircraft excluded
